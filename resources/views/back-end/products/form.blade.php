...
<div class="form-group">
    ...
    <div class="col-lg-10">
        <input type="text" name="{{ $input }}" value="{{ isset($row) ? $row->{$input} :0 }}" class="form-control"
            required style="width: 420px; height: 40px" id="purchasing_price" onkeyup="calculate_total_price()">
        @error($input)
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
...
@php $input = "selling_wholesale_price"; @endphp
<div class="form-group">
    <label class="col-lg-2 control-label">سعر البيع(جملة)</label>
    <div class="col-lg-10">
        <input type="text" name="{{ $input }}" value="{{ isset($row) ? $row->{$input} : '' }}" class="form-control"
            required style="width: 420px; height: 40px">
        @error($input)
...
@php $input = "selling_sectoral_price"; @endphp
<div class="form-group">
    <label class="col-lg-2 control-label"> سعر البيع(قصطاعي)</label>
    <div class="col-lg-10">
        <input type="text" name="{{ $input }}" value="{{ isset($row) ? $row->{$input} : '' }}" class="form-control"
            required style="width: 420px; height: 40px">
        @error($input)
...
<div class="form-group">
    ...
    <div class="col-lg-10">
        <input type="quantity" name="{{ $input }}" value="{{ isset($row) ? $row->{$input} :0}}" class="form-control"
            required style="width: 420px; height: 40px" id="quantity" onkeyup="calculate_total_price()">
        @error($input)
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
